...
void SensorControllerPanel::SetClient(const Pine::Ref<Client>& client)
{
    m_Client = client;
}
...
void SensorControllerPanel::UpdateImageTexture()
{
    ...
    {
        m_ImageTexture = Pine::Texture2D::Create(m_Image->GetWidth(),
            m_Image->GetHeight(),
...
{
    PINE_INFO("Sensor Controller Accepted!");
}

void SensorControllerPanel::OnSensorControllerDeny(
    Pine::TCP::Message<MessageTypes>& message)
{
    PINE_INFO("Sensor Controller Denied!");
}

void SensorControllerPanel::OnImage(Pine::TCP::Message<MessageTypes>& message)
{
    uint32_t width, height, channels;
    message >> channels >> height >> width;
    auto image = Pine::CreateRef<Image<uint8_t>>(width, height, channels);
    message >> image->GetBuffer();
    m_Image = image;
}

void SensorControllerPanel::OnImageDeny(
    Pine::TCP::Message<MessageTypes>& message)
{
    // TODO: Implement.
}

void SensorControllerPanel::OnImageStream(
    Pine::TCP::Message<MessageTypes>& message)
{
...
void SensorControllerPanel::OnImageStreamDeny(
    Pine::TCP::Message<MessageTypes>& message)
{
    // TODO: Implement.
}
