...
void InitParametersPanel::OnImGuiRender()
{
    if (ImGui::Begin("Initialization Parameters"))
    {
        ...
        Resolution resOptions[] = {Resolution::None,
            Resolution::HD2K,
            Resolution::HD1080,
            Resolution::HD720,
            Resolution::VGA};
        ...
        const char* depthLabels[] = {"None", "Performance", "Quality", "Ultra"};
        ...
        if (ImGui::BeginCombo("Depth Mode", depthLabel))
        {
            for (int n = 0; n < IM_ARRAYSIZE(depthLabels); n++)
            {
                ...
                if (ImGui::Selectable(depthLabels[n], isSelected))
                {
                    depthIndex = n;
                    m_Parameters.depthMode = depthOptions[n];
                }
        ...
        const char* unitLabels[] =
            {"None", "Millimeter", "Centimeter", "Meter", "Inch", "Foot"};
        Unit unitOptions[] = {Unit::None,
            Unit::Millimeter,
            Unit::Centimeter,
            Unit::Meter,
            Unit::Inch,
        ...
        if (ImGui::BeginCombo("Coordinate Units", unitLabel))
        {
            ...
            {
                const bool isSelected =
                    (m_Parameters.coordinateUnits == unitOptions[n]);

                if (ImGui::Selectable(unitLabels[n], isSelected))
                {
                    unitIndex = n;
                    m_Parameters.coordinateUnits = unitOptions[n];
                }
        ...
        static const char* sysLabels[] = {"None",
            "Image",
            "LeftHandedYUp",
            "RightHandYUp",
            "RightHandedZUp",
        ...
        ImGui::SliderFloat("Minimum Depth",
            &m_Parameters.minDepth,
            -1.0f,
            m_Parameters.maxDepth,
            "%.1f");
        ImGui::SliderFloat("Maximum Depth",
            &m_Parameters.maxDepth,
            m_Parameters.minDepth,
            10000.0f,
            "%.1f");
